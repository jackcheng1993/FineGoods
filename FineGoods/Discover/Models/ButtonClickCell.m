...
@implementation ButtonClickCell {
    NSString *_url;
...
- (void)setModelbtn:(ButtonListModel *)modelbtn {
    
    _content.text = modelbtn.content;
    
    _picArray = modelbtn.pics;
    PicsModel *picsModel = _picArray[0];
    [_bigImage sd_setImageWithURL:[NSURL URLWithString:picsModel.url]];
    
   // [_buyImage setImage:[UIImage imageNamed:@"Fplaceholder"]];
    
    _buyPicsArray = modelbtn.product;
    if (_buyPicsArray.count == 1) {
      ProductModel *producsModel = _buyPicsArray[0];
